...
class Quaternion(object):
    """Quaternion object implementation without numpy"""
    ...
    def __abs__(self):
        ...
        """
        return sqrt(sum([x*x for x in self.internal]))
    ...
    def norm(self):
        elements = self.internal[:]
        length = sqrt(sum([element*element for element in elements]))
        if length != 1:
            elements = [element/length for element in elements]
